            ...
            for (let trigger of TRIGGERS) {
                if (i + trigger.length <= tokens.length) {
                    ...
                    for (let j = 0; j < trigger.length; j++) {
                        ...
                        if (tStr !== trigger[j]) { match = false; break; }
                        if (tokens[i+j].includes('(') || tokens[i+j].includes(')')) { match = false; break; }
                    }
                    if (match) {
...
files.forEach(file => {
    ...
    let newContent = content.replace(/"([^"]*)"|'([^']*)'/g, (fullMatch, doubleQuoted, singleQuoted) => {
